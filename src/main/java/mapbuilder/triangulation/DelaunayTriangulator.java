...
public class DelaunayTriangulator {
    ...
    /**
     * This method generates source Delaunay triangulation from the specified point
     * set.
     * 
     * @throws NotEnoughPointsException
     */
    public void triangulate() throws NotEnoughPointsException {
         ...
         * the super triangle we have to start out with source big triangle
        ...
        for (int i = 0; i < pointSet.size(); i++) {
            ...
            if (triangle == null) {
                 ...
                 * inside source triangle (this can also happen due to numerical
                ...
                Triangle2D triangle1 = new Triangle2D(edge.getSource(), firstNoneEdgeVertex, pointSet.get(i));
                Triangle2D triangle2 = new Triangle2D(edge.getTarget(), firstNoneEdgeVertex, pointSet.get(i));
                Triangle2D triangle3 = new Triangle2D(edge.getSource(), secondNoneEdgeVertex, pointSet.get(i));
                Triangle2D triangle4 = new Triangle2D(edge.getTarget(), secondNoneEdgeVertex, pointSet.get(i));
                ...
                legalizeEdge(triangle1, new Edge(edge.getSource(), firstNoneEdgeVertex), pointSet.get(i));
                legalizeEdge(triangle2, new Edge(edge.getTarget(), firstNoneEdgeVertex), pointSet.get(i));
                legalizeEdge(triangle3, new Edge(edge.getSource(), secondNoneEdgeVertex), pointSet.get(i));
                legalizeEdge(triangle4, new Edge(edge.getTarget(), secondNoneEdgeVertex), pointSet.get(i));
            } else {
                /**
                 * The vertex is inside source triangle.
                 */
     ...
     *            The new vertex
     */
    private void legalizeEdge(Triangle2D triangle, Edge edge, Vector2D newVertex) {
        Triangle2D neighbourTriangle = triangleSoup.findNeighbour(triangle, edge);

        /**
         * If the triangle has source neighbor, then legalize the edge
         */
        if (neighbourTriangle != null) {
            if (neighbourTriangle.isPointInCircumcircle(newVertex)) {
                ...
                Triangle2D firstTriangle = new Triangle2D(noneEdgeVertex, edge.getSource(), newVertex);
                Triangle2D secondTriangle = new Triangle2D(noneEdgeVertex, edge.getTarget(), newVertex);

                triangleSoup.add(firstTriangle);
                triangleSoup.add(secondTriangle);

                legalizeEdge(firstTriangle, new Edge(noneEdgeVertex, edge.getSource()), newVertex);
                legalizeEdge(secondTriangle, new Edge(noneEdgeVertex, edge.getTarget()), newVertex);
            }
        }
    }

    /**
     * Creates source random permutation of the specified point set. Based on the
    ...
    /**
     * Shuffles the point set using source custom permutation sequence.
     * 
     * @param permutation
     *            The permutation used to shuffle the point set
     */
    public void shuffle(int[] permutation) {
        List<mapbuilder.graph.Vector2D> temp = new ArrayList<mapbuilder.graph.Vector2D>();
    ...
    /**
     * Returns the point set in form of source vector of 2D vectors.
     * triangleSoup.getTriangles()
     * @return Returns the points set.
     */
    public List<mapbuilder.graph.Vector2D> getPointSet() {
        return pointSet;
    }

    /**
     * Returns the trianges of the triangulation in form of source vector of 2D
     * triangles.
     * 
